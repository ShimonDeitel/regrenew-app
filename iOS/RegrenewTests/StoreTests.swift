...
final class StoreTests: XCTestCase {
    ...
    func testAddIncreasesCount() {
        ...
        let ok = store.add(RegrenewEntry(title: "Test"))
        XCTAssertTrue(ok)
    ...
    func testAddFailsAtFreeLimit() {
        ...
        while store.entries.count < Store.freeLimit {
            _ = store.add(RegrenewEntry(title: "Filler"))
        }
        let result = store.add(RegrenewEntry(title: "Overflow"))
        XCTAssertFalse(result)
    }

    func testDeleteRemovesEntry() {
        let store = Store()
        _ = store.add(RegrenewEntry(title: "ToDelete"))
    ...
    func testUpdateChangesTitle() {
        let store = Store()
        _ = store.add(RegrenewEntry(title: "Original"))
    ...
    func testDeleteAtOffsets() {
        let store = Store()
        let before = store.entries.count
        _ = store.add(RegrenewEntry(title: "OffsetTest"))
