...
struct ContentView: View {
    ...
    @State private var editingEntry: RegrenewEntry?
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Registration Renewal")
            .toolbar {
...
enum EntryFormMode {
    case add
    case edit(RegrenewEntry)
}

struct EntryFormView: View {
    let mode: EntryFormMode
    var onSave: (RegrenewEntry) -> Void
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("State", text: $field2)
                        .accessibilityIdentifier("field2Field")
                        .focused($focusedField, equals: .field2)
                    TextField("Plate #", text: $field3)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry: RegrenewEntry
                        if case .edit(let existing) = mode {
                            entry = existing
                        } else {
                            entry = RegrenewEntry(title: "")
                        }
            ...
            .onAppear {
                if case .edit(let existing) = mode {
                    ...
                    date = existing.date
                    field2 = existing.state
                    field3 = existing.plate
                    note = existing.note
                }
            }
        }
